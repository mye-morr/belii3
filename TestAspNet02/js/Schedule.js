class Schedule {
    ...
    newSchedItem(inItem, inEncrypt, pointValAdj, cumPts) {
    ...
    writeSched(hasDetails, hasNoTime) {
        ...
        var time = '';
        var arrBuf, arrTimeEncryptKey;
        html += '<details><summary>schedule</summary><br/><table class="schedule">'

        for (var i = 0; i < arrSchedule.length; i++) {
            if (!(arrSchedule[i]._cumPts == '-') // info-button
                || hasDetails == 1) {
                ...
                if (hasNoTime == 1)
                    ...
                    arrBuf = [0, 0, 0, 0, 0];
                    arrTimeEncryptKey = arrSchedule[i]._arrReverseTimeEncryptKey.reverse();
